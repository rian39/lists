---
title: "'Have you forgotten?' Conversion experiences in lists"
...
# 'Have you forgotten?' Conversion experiences in lists
...
ggplot(df, aes(x=price,y=ContentsQuantity)) + geom_point() + theme(axis.text=element_text(size=10)) 
kable(df[df$price>=20,'name'])
```
...
![The advent of recommender systems](figure/resnick_varian_page1_197.png)
...
```{r arules, cache=TRUE, echo=TRUE, fig.margin=TRUE}
library(knitr)
library(arules)
library(arulesViz)
library(datasets)
...
```{r tesco, cache=TRUE, fig.cap ='Tesco surplus',  fig.margin=TRUE}
    library(ggplot2)
...
People inhabit lists and work with them no matter how they are made; thus vindicating ethnomethodologists assertion that social order is the product of ongoing sense-making practices. [^45] But it also suggests a transformation in the way that lists exist in the world. They are less the inscriptions of a discrete agent than an effect of an environment teeming with possible combinations and complementaries, a surplus of possible purchases that the supermarket needs to continually maintain but also filter in order to make it possible for something to happen. 
...
We have seen two different ways in which lists might be constructed in a recommender system. 
...
The fact that so little was said about the models themselves suggests that they are now largely taken for granted in recommender systems. The models she mentioned are all classifiers, predictive models that treat a set of variables in a datasets as a vector space in which good classification means finding a way of defining a decision boundary between points that belong to different classes of things. Here the classes of things are binary: `recommended` or `not recommended.` Some classifier models, including logistic regression, generate a probability rather than simply a classification.  
...
```{r sparsity, echo=FALSE, cache=TRUE, fig.margin=TRUE, fig.cap ='The sparsity of data' }
library(ggplot2)
library(reshape2)
...
- listing practices are very closely entangled with problems of social order under predictive conditions or amidst predictive operations. 
