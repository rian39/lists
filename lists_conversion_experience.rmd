...
```{r simulate_basket, echo=FALSE, cache=TRUE, fig.margin=TRUE}
library(dplyr)
library(ggplot2)
library(readr)
...
```{r arules, echo=TRUE, fig.margin=TRUE}
library(arules)
library(arulesViz)
library(datasets)
data(Groceries)
...
Hansen refers to an ontological transformation in which predictive media or recommendation systems would be seen as the closed realisation of the actually open propensities of the world. By understanding the world in terms of propensities, he argues, we have a change to engage with contemporary predictive media. 
