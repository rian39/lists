...
How should we view the predictive re-writing of lists? The media theorist Mark Hansen has recently suggested that 
...
Hansen's account resonates with wider views of big data.  The expectation energising much predictive practice  pivots on 'micrological propensities' elicited from data.  Hansen  links his Whitehead-inspired ontology to propensities of things in the world via probabilities:
...
Hansen predicates prediction as an 'ontological transformation' that deploys probabilities in an evermore closely woven and encompassing expression of the animated, eventful, propensities of things. Predictive analytics links probabilities as calculations to propensities or the mutable agencies of things. The practical and indeed empirical  question is whether such transformations in probability that bring 'the propensities of things'  can be detected and articulated in the prosaic setting of shopping lists and recommender systems.[^20.1]
...
Figure \@ref(fig:simulate-inventory) plots something of the density of commodities in the Tesco grocery database. By virtue of their sheer abundance, the plot cannot easily separate the many items that Tesco sells, especially in the price range £1-£10. A sample of the most highly priced items in Tesco stores is shown in Table  \@ref(tab:expensive-groceries)      
...
```{r arules-ex, cache=TRUE, message=FALSE, echo=FALSE, fig.margin=TRUE}
library(knitr)
library(arules)
library(arulesViz)
library(datasets)
